...
class playerClass:
    def __init__(self, characterName):
        ...
        self.baseHealth = 12 + self.classStats(characterName)["base"]["health"]
    ...
    def rollMove(self, playerMove=False):
        if not playerMove:
            playerMove = random.randint(1,12) + self.calcStat("move")
        return playerMove

    def rollDamage(self, playerDamage=False):
        if not playerDamage:
            playerDamage = random.randint(1,12) + self.calcStat("damage") 
        return playerDamage